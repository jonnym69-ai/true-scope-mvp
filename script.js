...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (generateBtn) {
        generateBtn.addEventListener('click', async function() {
            ...
            const userLimit = DAILY_LIMITS[userSubscription] || DAILY_LIMITS.free;
            if (userUsageToday >= userLimit) {
                const conf = document.getElementById('confirmation');
                if (userSubscription === 'free') {
                    conf.innerHTML = '⚠️ Daily limit reached! <a href="#" onclick="document.getElementById(\'upgrade-btn\').click()">Upgrade to Premium</a> for more attempts.';
                } else {
                    conf.innerHTML = '⚠️ Daily limit reached! Upgrade to a higher tier for more attempts.';
                }
...
// Export to GDD function
function exportToGDD() {
    const element = document.getElementById('output');
    if (!element || !element.innerHTML.trim()) {
        alert('No content to export. Generate a plan first!');
        return;
    ...
    const gddContent = `GAME DESIGN DOCUMENT\n\nGenerated by True Scope Gaming\n\n${content}\n\n--- END OF DOCUMENT ---\n\nCreated with True Scope Gaming - Making games made simple!`;
    
    const blob = new Blob([gddContent], { type: 'text/plain;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'game-design-document.txt';

...
let userExportUsageToday = 0;
const DAILY_LIMITS = {
    free: 3,      // 3 free attempts per day
    premium: 15,  // 15 for premium subscribers
    pro: 30,      // 30 for pro
    studio: 50,   // 50 for studio
    enterprise: 100 // 100 for enterprise
};
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Export to GDD button
    const exportGddBtn = document.getElementById('export-gdd');
    if (exportGddBtn) {
        exportGddBtn.addEventListener('click', exportToGDD);
    }
...
function exportToPDF() {
    ...
    if (!element || !element.innerHTML.trim()) {
        alert('No content to export. Generate a plan first!');
        return;
    }
    
    // Check export limit for free users
    if (userSubscription === 'free' && userExportUsageToday >= 3) {
        alert('Export limit reached! Upgrade to Premium for unlimited PDF and GDD exports.');
        return;
    }
    ...
    // Increment export usage for free users
    if (userSubscription === 'free') {
        userExportUsageToday++;
        saveUserData();
    }
}
...
function exportToMarkdown() {
    // Check subscription for Markdown export (Pro+)
    if (userSubscription !== 'pro' && userSubscription !== 'studio' && userSubscription !== 'enterprise') {
        alert('Markdown export is available for Pro and higher tiers. Upgrade to unlock this feature.');
        return;
    }
    ...
    a.href = url;
    a.download = 'game-development-plan.md';
...
function exportToJSON() {
    // Check subscription for JSON export (Studio+)
    if (userSubscription !== 'studio' && userSubscription !== 'enterprise') {
        alert('JSON export is available for Studio and higher tiers. Upgrade to unlock this feature.');
        return;
    }
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = 'game-development-plan.json';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    ...
    try {
        // Determine number of variations based on subscription
        let numVariations = 3;
        if (userSubscription === 'premium') numVariations = 5;
        if (userSubscription === 'pro') numVariations = 10;
        if (userSubscription === 'studio') numVariations = 15;
        if (userSubscription === 'enterprise') numVariations = 20;
...
Format this as a professional technical specification document.`;
                ...
                default: // standard professional plan
                    userPrompt = `Generate ${numVariations} comprehensive professional game development plans for: "${userIdea}"

Each plan should be a complete Game Design Document with the following sections:
...
async function generateFallbackPlan(userIdea, tool, planType) {
    // Determine number of variations based on subscription (same as AI)
    let numVariations = 3;
    if (userSubscription === 'premium') numVariations = 5;
    if (userSubscription === 'pro') numVariations = 10;
    if (userSubscription === 'studio') numVariations = 15;
...
function generatePlanVariations(keywords, tool, numVariations = 30) {
    const variations = [];
    
    // Expand keywords with defaults for variety
    const genres = keywords.genres.length > 0 ? keywords.genres : ['adventure', 'puzzle', 'rpg', 'shooter', 'racing', 'simulation', 'platformer', 'strategy', 'horror', 'casual'];
    const styles = keywords.styles.length > 0 ? keywords.styles : ['space', 'fantasy', 'cyberpunk', 'medieval', 'modern', 'futuristic', 'retro', 'colorful', 'dark', 'abstract'];
    const mechanics = keywords.mechanics.length > 0 ? keywords.mechanics : ['match-3', 'physics', 'crafting', 'exploration', 'combat', 'stealth', 'platforming', 'racing', 'simulation', 'puzzle-solving'];
...
async function loadUserData() {
    ...
    try {
        ...
        if (userDoc.exists()) {
            ...
            userExportUsageToday = userData.exportUsageToday || 0;
            
            console.log('User data loaded:', {
                subscription: userSubscription,
                usageToday: userUsageToday,
                exportUsageToday: userExportUsageToday,
            ...
            if (lastUsageDate !== today) {
                userUsageToday = 0;
                userExportUsageToday = 0;
                await updateUserData({ usageToday: 0, exportUsageToday: 0, lastUsageDate: today });
            }
        } else {
            // New user
...
function updateUIForUser() {
    ...
    // Hide/show export buttons based on subscription
    const exportMarkdownBtn = document.getElementById('export-markdown');
    if (exportMarkdownBtn) {
        exportMarkdownBtn.style.display = (userSubscription === 'pro' || userSubscription === 'studio' || userSubscription === 'enterprise') ? 'inline-block' : 'none';
    }
    
    const exportJsonBtn = document.getElementById('export-json');
    if (exportJsonBtn) {
        exportJsonBtn.style.display = (userSubscription === 'studio' || userSubscription === 'enterprise') ? 'inline-block' : 'none';
    }
